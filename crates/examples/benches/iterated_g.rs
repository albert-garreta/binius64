...
//! Iterated g benchmark for $g(x) = (f(x) \cdot (2^{32\cdot 7} - 1))^2 \bmod q$.

// cargo bench -p binius-examples --bench iterated_g

mod utils;

use std::alloc::System;

use binius_examples::circuits::iterated_g::{DEFAULT_ITERATIONS, Instance, IteratedGExample, Params};
...
#[global_allocator]
static ITERATED_G_PEAK_ALLOC: PeakAlloc<System> = PeakAlloc::new(System);

struct IteratedGBenchmark {
	log_inv_rate: usize,
	iterations: usize,
}

impl IteratedGBenchmark {
	fn new() -> Self {
...
impl ExampleBenchmark for IteratedGBenchmark {
	type Params = Params;
	type Instance = Instance;
	type Example = IteratedGExample;
	...
	fn print_params(&self) {
		...
		print_benchmark_header("Iterated g", &params_list);
	}
}

fn bench_iterated_g(c: &mut Criterion) {
	let benchmark = IteratedGBenchmark::new();
	run_cs_benchmark(c, benchmark, "iterated_g", &ITERATED_G_PEAK_ALLOC);
}

criterion_group!(benches, bench_iterated_g);
